""
function run_mc_opf_bf_del_mx1(data::Dict{String,Any}, model_constructor, solver; kwargs...)
    return _PMs.run_model(data, model_constructor, solver, post_mc_opf_bf_del_mx1; solution_builder=solution_tp!, multiconductor=true, kwargs...)
end


""
function run_mc_opf_bf_del_mx1(file::String, model_constructor, solver; kwargs...)
    return run_mc_opf_bf_del_mx1(PowerModelsDistribution.parse_file(file), model_constructor, solver; kwargs...)
end


""
function post_mc_opf_bf_del_mx1(pm::_PMs.GenericPowerModel)
    # Variables
    ...
    for i in _PMs.ids(pm, :load)
        ...
        constraint_mc_load_mx_SWL_only_delta(pm, i)
    end

    for i in _PMs.ids(pm, :gen)
        #constraint_mc_generation_mx_SWL(pm, i)
    end
