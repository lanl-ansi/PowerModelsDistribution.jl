...
function run_mc_model(data::Dict{String,<:Any}, model_type::Type, solver, build_mc::Function; ref_extensions::Vector{<:Function}=Vector{Function}([]), make_si=!get(data, "per_unit", false), multinetwork::Bool=false, kwargs...)::Dict{String,Any}
    if get(data, "data_model", MATHEMATICAL) == ENGINEERING
        ...
        result = _PM.run_model(data_math, model_type, solver, build_mc; ref_extensions=[ref_add_arcs_transformer!, ref_add_converter_storage!, ref_add_bus_converter!, ref_extensions...], multiconductor=true, multinetwork=multinetwork, kwargs...)
    ...
    elseif get(data, "data_model", MATHEMATICAL) == MATHEMATICAL
        result = _PM.run_model(data, model_type, solver, build_mc; ref_extensions=[ref_add_arcs_transformer!, ref_add_converter_storage!, ref_add_bus_converter!, ref_extensions...], multiconductor=true, multinetwork=multinetwork, kwargs...)
    end
