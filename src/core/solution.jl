...
function get_solution_vseq(pm::GenericPowerModel, sol::Dict{String,<:Any})
...
function get_solution_vseq(pm::GenericPowerModel, nw::Int, sol_nw::Dict{String,<:Any})
    PMs.get_solution(pm, sol_nw)
    if !ismulticonductor(pm, nw=nw) && ref(pm, nw, :conductors)==3
        error(LOGGER, "Sequence components are only defined on three-phase networks.")
    end
