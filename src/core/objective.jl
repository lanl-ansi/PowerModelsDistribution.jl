    ...
    JuMP.@objective(pm.model, Min,
        sum(
            sum(
                ...
                sum( (M[n][c]*(1 - _PMs.var(pm, n, :z_shunt, i)))^2 for (i,shunt) in nw_ref[:shunt]) +
                sum( (M[n][c]*(1 - _PMs.var(pm, n, :z_gen, i))^2 for (i,gen) in nw_ref[:gen])) +
                sum( (M[n][c]*(1 - _PMs.var(pm, n, :z_storage, i))^2 for (i,storage) in nw_ref[:storage]))
            for c in _PMs.conductor_ids(pm, n))
        for (n, nw_ref) in _PMs.nws(pm))
    )
end
