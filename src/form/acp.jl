...
function variable_mc_bus_voltage(pm::_PM.AbstractACPModel; nw=pm.cnw, kwargs...)
    ...
    bus_t1 = [bus for (_, bus) in ref(pm, nw, :bus) if bus["bus_type"]==3]
    if length(bus_t1)>0
        theta = bus_t1[1]["va"]
    else
    ...
    for id in ids(pm, nw, :bus)
        ...
        if !haskey(busref, "va_start")
            ...
            JuMP.set_start_value.(var(pm, nw, :va, id), theta)
        end
    end
end
...
function variable_mc_bus_voltage_on_off(pm::_PM.AbstractACPModel; kwargs...)
    ...
    for id in ids(pm, nw, :bus)
        ...
        # if it has this key, it was set at PM level
            for c in 1:ncnd
                JuMP.set_start_value(var(pm, nw, :va, id)[c], theta[c])
            end
        end
    end
end
...
""
function constraint_mc_slack_power_balance(pm::_PM.AbstractACPModel, nw::Int, i::Int, bus_arcs, bus_arcs_sw, bus_arcs_trans, bus_gens, bus_converter, bus_pd, bus_qd, bus_gs, bus_bs)
    vm   = var(pm, nw, :vm, i)
    va   = var(pm, nw, :va, i)
...
""
function constraint_mc_shed_power_balance(pm::_PM.AbstractACPModel, nw::Int, i::Int, bus_arcs, bus_arcs_sw, bus_arcs_trans, bus_gens, bus_converter, bus_pd, bus_qd, bus_gs, bus_bs)
    vm       = var(pm, nw, :vm, i)
    va       = var(pm, nw, :va, i)
    p        = get(var(pm, nw),    :p, Dict()); _PM._check_var_keys(p, bus_arcs, "active power", "branch")
    q        = get(var(pm, nw),    :q, Dict()); _PM._check_var_keys(q, bus_arcs, "reactive power", "branch")
...
""
function constraint_mc_power_balance(pm::_PM.AbstractACPModel, nw::Int, i::Int, bus_arcs, bus_arcs_sw, bus_arcs_trans, bus_gens, bus_converter, bus_pd, bus_qd, bus_gs, bus_bs)
    vm   = var(pm, nw, :vm, i)
    va   = var(pm, nw, :va, i)
    p    = get(var(pm, nw),    :p, Dict()); _PM._check_var_keys(p, bus_arcs, "active power", "branch")
    q    = get(var(pm, nw),    :q, Dict()); _PM._check_var_keys(q, bus_arcs, "reactive power", "branch")
...
""
function constraint_mc_load_power_balance(pm::_PM.AbstractACPModel, nw::Int, i::Int, bus_arcs, bus_arcs_sw, bus_arcs_trans, bus_gens, bus_converter, bus_loads, bus_gs, bus_bs)
    vm   = var(pm, nw, :vm, i)
    va   = var(pm, nw, :va, i)
    p    = get(var(pm, nw),   :p, Dict()); _PM._check_var_keys(p, bus_arcs, "active power", "branch")
    q    = get(var(pm, nw),   :q, Dict()); _PM._check_var_keys(q, bus_arcs, "reactive power", "branch")
...
```
"""
function constraint_mc_ohms_yt_from(pm::_PM.AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_fr, b_fr, tr, ti, tm)
...
```
"""
function constraint_mc_ohms_yt_to(pm::_PM.AbstractACPModel, n::Int, f_bus, t_bus, f_idx, t_idx, g, b, g_to, b_to, tr, ti, tm)
...
function constraint_mc_transformer_power_yy(pm::_PM.AbstractACPModel, nw::Int, trans_id::Int, f_bus::Int, t_bus::Int, f_idx, t_idx, f_cnd, t_cnd, pol, tm_set, tm_fixed, tm_scale)
    ...
    va_to = var(pm, nw, :va, t_bus)[t_cnd]
...
function constraint_mc_transformer_power_dy(pm::_PM.AbstractACPModel, nw::Int, trans_id::Int, f_bus::Int, t_bus::Int, f_idx, t_idx, f_cnd, t_cnd, pol, tm_set, tm_fixed, tm_scale)
    ...
    va_to = [var(pm, nw, :va, t_bus)[p] for p in t_cnd]
...
""
function constraint_mc_converter_losses(pm::_PM.AbstractACPModel, n::Int, i, bus, r, x, p_loss, q_loss, conductors)
    vm = var(pm, n, :vm, bus)
    ps = var(pm, n, :ps, i)
    qs = var(pm, n, :qs, i)
    pdc = var(pm, n, :pdc, i)
