...
function variable_mx_real(pm::PMs.GenericPowerModel, indices::Array{T,1}, N::Int,
        lower_bound::Dict{T,Array{B,2}},
        upper_bound::Dict{T,Array{B,2}};
        name="", prefix="") where {T, B<:Real}
...
function variable_mx_complex(pm::PMs.GenericPowerModel, indices::Array{T,1}, N::Int,
        lower_bound::Dict{T,Array{B,2}}, upper_bound::Dict{T,Array{B,2}};
    ...
    Mim = variable_mx_real(pm, indices, N,
        lower_bound=lower_bound, upper_bound=upper_bound, prefix=prefix,
        name=name_imag
    )
    return (Mre,Mim)
end
...
"""
Often, lower_bound=-upper_bound, this does just that:
    upper_bound=bound
    lower_bound=-bound
"""
function variable_mx_complex(pm::PMs.GenericPowerModel, indices::Array{T,1}, N::Int,
...
useful for imaginary part of Hermitian matrices.
"""
function variable_mx_skewsymmetric(pm::PMs.GenericPowerModel, indices::Array{T,1}, N::Int,
        lower_bound::Dict{T,Array{B,2}},
        upper_bound::Dict{T,Array{B,2}};
        diag_is_zero=false, name="", prefix="") where {T, B<:Real}
    # the output is a dictionary of (index, matrix) pairs
    # the data type of the matrix has to be JuMP.GenericAffExpr, because it
    # will also contain inverted variables (upper triangle) and potentially
    # constants (zero) on the diagonal
...
function variable_mx_hermitian(pm::PMs.GenericPowerModel, indices::Array{T,1}, N::Int,
    ...
    Mim = variable_mx_skewsymmetric(pm, indices, N; diag_is_zero=true,
        lower_bound=lower_bound, upper_bound=upper_bound, prefix=prefix,
        name=name_imag
    )
    return (Mre,Mim)
