...
function parse_matlab_string(data_string::String)
    ...
    if haskey(matlab_data, "tppmc.bus")
        ...
        for bus_row in matlab_data["tppmc.bus"]
            bus_data = InfrastructureModels.row_to_typed_dict(bus_row, _tppm_bus_columns)
    ...
    if haskey(matlab_data, "tppmc.load")
        ...
        for (i,load_row) in enumerate(matlab_data["tppmc.load"])
            load_data = InfrastructureModels.row_to_typed_dict(load_row, _tppm_load_columns)
            load_data["index"] = i
    ...
    if haskey(matlab_data, "tppmc.shunt")
        ...
        for (i,shunt_row) in enumerate(matlab_data["tppmc.shunt"])
            shunt_data = InfrastructureModels.row_to_typed_dict(shunt_row, _tppm_shunt_columns)
            shunt_data["index"] = i
    ...
    if haskey(matlab_data, "tppmc.gen")
        ...
        for (i, gen_row) in enumerate(matlab_data["tppmc.gen"])
            gen_data = InfrastructureModels.row_to_typed_dict(gen_row, _tppm_gen_columns)
            gen_data["index"] = i
            push!(gens, gen_data)
        end
        case["gen"] = gens
    ...
    if haskey(matlab_data, "tppmc.branch")
        ...
        for (i, branch_row) in enumerate(matlab_data["tppmc.branch"])
            branch_data = InfrastructureModels.row_to_typed_dict(branch_row, _tppm_branch_columns)
            branch_data["index"] = i
    ...
    if haskey(matlab_data, "tppmc.bus_name")
        ...
        for (i, bus_name_row) in enumerate(matlab_data["tppmc.bus_name"])
            bus_name_data = InfrastructureModels.row_to_typed_dict(bus_name_row, _tppm_bus_name_columns)
    ...
    for k in keys(matlab_data)
        if !in(k, _tppm_data_names) && startswith(k, "tppmc.")
