    ...
    for (name, ravens_obj) in get(conducting_equipment, "PowerTransformer", Dict{Any,Dict{String,Any}}())
        ...
        if haskey(ravens_obj, "PowerTransformer.PowerTransformerEnd")
            ...
            for wdg_id in 1:nrw
                ...
                transformer_2wa_obj["tm_lb"] = 0.9 * ones(length(transformer_2wa_obj[f_connections]))
                transformer_2wa_obj["tm_ub"] = 1.1 * ones(length(transformer_2wa_obj[f_connections]))
        ...
        elseif haskey(ravens_obj, "PowerTransformer.TransformerTank")
            ...
            for wdg_id in 1:nrw
                ...
                #transformer_2wa_obj["tm_lb"] = tm_lb[wdg_id]
                #transformer_2wa_obj["tm_ub"] = tm_ub[wdg_id]
                transformer_2wa_obj["tm_lb"] = 0.9 * ones(size(transformer_2wa_obj[f_connections]))
                transformer_2wa_obj["tm_ub"] = 1.1 * ones(size(transformer_2wa_obj[f_connections]))
                transformer_2wa_obj["tm_step"] = tm_step[wdg_id]
