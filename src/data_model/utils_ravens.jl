...
function _calc_shunt_admittance_matrix(terminals, b)
    ...
    #this is only correct for a y-grounded per phase shunt admittance to ground
    #ignores type of shunt completely
    N = length(terminals)
...
#TODO: implement other methods for calculating shunt admittance matrix
...
function _recursive_dict_get(dict::Dict, path::Vector{<:Any}, default::Any)::Any
    if length(path) > 1
        return _recursive_dict_get(Dict(get(dict, path[1], Dict())), path[2:end], default)
    else
