...
@testset "test loadmodels pf" begin
    @testset "loadmodels connection variations" begin
        ...
        @test isapprox(pd(sol, pmd, "d1ph"), [0.4000, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1ph"), [0.3000, 0, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "d1ph2"), [0, 0.4000, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1ph2"), [0, 0.3000, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "d1ph23"), [0, 0.2866, 0.1134], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1ph23"), [0, 0.0345, 0.2655], atol=1E-4)
        @test isapprox(pd(sol, pmd, "d1ph0"), [0, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1ph0"), [0, 0, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "d1ph00"), [0, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1ph00"), [0, 0, 0], atol=1E-4)
        # Leads to an error now instead; order of conductors needed for this
        # @test isapprox(pd(sol, pmd, "d1ph123"), [0.4000, 0, 0], atol=1E-4)
        # @test isapprox(qd(sol, pmd, "d1ph123"), [0.3000, 0, 0], atol=1E-4)
        # single-phase wye loads
        # Leads to an error now instead; order of conductors needed for this
        # @test isapprox(pd(sol, pmd, "y1ph"), [0.4000, 0, 0], atol=1E-4)
        # @test isapprox(qd(sol, pmd, "y1ph"), [0.3000, 0, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "y1ph2"), [0, 0.4000, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "y1ph2"), [0, 0.3000, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "y1ph23"), [0, 0.2866, 0.1134], atol=1E-4)
        @test isapprox(qd(sol, pmd, "y1ph23"), [0, 0.0345, 0.2655], atol=1E-4)
        @test isapprox(pd(sol, pmd, "y1ph0"), [0, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "y1ph0"), [0, 0, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "y1ph00"), [0, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "y1ph00"), [0, 0, 0], atol=1E-4)
        # Leads to an error now instead; order of conductors needed for this
        # @test isapprox(pd(sol, pmd, "y1ph123"), [0.4000, 0, 0], atol=1E-4)
        # @test isapprox(qd(sol, pmd, "y1ph123"), [0.3000, 0, 0], atol=1E-4)
        # three-phase loads
        ...
        @test isapprox(pd(sol, pmd, "d3ph1230"), [0.1333, 0.1333, 0.1333], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d3ph1230"), [0.100, 0.100, 0.100], atol=1E-4)
        ...
        @test isapprox(pd(sol, pmd, "d3ph3120"), [0.1333, 0.1333, 0.1333], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d3ph3120"), [0.100, 0.100, 0.100], atol=1E-4)
    end
    @testset "loadmodels 1/2/5 in acp pf" begin
        ...
        @test isapprox(pd(sol, pmd, "d1phm1"), [0.4000, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1phm1"), [0.3000, 0, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "d1phm2"), [0.2783, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1phm2"), [0.2087, 0, 0], atol=1E-4)
        @test isapprox(pd(sol, pmd, "d1phm5"), [0.3336, 0, 0], atol=1E-4)
        @test isapprox(qd(sol, pmd, "d1phm5"), [0.2502, 0, 0], atol=1E-4)
