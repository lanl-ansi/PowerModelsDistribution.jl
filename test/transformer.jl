...
@testset "test transformer pf" begin
    @testset "two winding transformer pf" begin
        ...
        @testset "2w transformer acp opf_oltc yy" begin
            ...
            pmd_data = PMD.parse_file(file)
            # free the taps
            pmd_data["transformer"]["1"]["fixed"] = MultiConductorVector(zeros(Bool, 3))
            pmd_data["transformer"]["2"]["fixed"] = MultiConductorVector(zeros(Bool, 3))
