...
tap(i, pm) = JuMP.value.(PM.var(pm, pm.cnw, :tap)[i])
...
pdvar(pm, pmd_data, name) = [PM.var(pm, pm.cnw, c, :pd, load_name2id(pmd_data, name)) for c in 1:3]
pd(sol, pmd_data, name) = sol["solution"]["load"][string(load_name2id(pmd_data, name))]["pd_bus"]
qdvar(pm, pmd_data, name) = [PM.var(pm, pm.cnw, c, :qd, load_name2id(pmd_data, name)) for c in 1:3]
