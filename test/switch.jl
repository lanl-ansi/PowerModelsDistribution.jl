...
@testset "test switch opf" begin
    @testset "3-bus balanced switch closed opf acp" begin
       result = run_mc_opf(eng, ACPPowerModel, ipopt_solver; make_si=false)
    ...
    @testset "3-bus balanced switch closed opf acr" begin
        result = run_mc_opf(eng, ACRPowerModel, ipopt_solver; make_si=false)
    ...
    @testset "3-bus balanced switch closed opf ivr" begin
        result = run_mc_opf(eng, IVRPowerModel, ipopt_solver; make_si=false)
    ...
    @testset "3-bus balanced switch closed opf nfa" begin
        result = run_mc_opf(eng, NFAPowerModel, ipopt_solver; make_si=false)
    ...
    @testset "3-bus balanced switch closed opf lpubfdiag" begin
        result = run_mc_opf(eng, LPUBFDiagPowerModel, ipopt_solver; make_si=false)
...
@testset "test switch pf" begin
    ...
    @testset "3-bus balanced switch closed pf ivr" begin
        result = run_mc_pf(eng, IVRPowerModel, ipopt_solver; make_si=false)
    ...
    @testset "3-bus balanced switch closed pf nfa" begin
        result = run_mc_pf(eng, NFAPowerModel, ipopt_solver; make_si=false)
...
@testset "test switch mld" begin
    ...
    @testset "3-bus balanced switch open mld acp" begin
        result = run_mc_mld(eng, ACPPowerModel, optimizer_with_attributes(Ipopt.Optimizer, "print_level"=>0, "tol"=>1e-6))
    ...
    @testset "3-bus balanced switch open mld acr" begin
        result = run_mc_mld(eng, ACRPowerModel, ipopt_solver)
    ...
    @testset "3-bus balanced switch open mld nfa" begin
        result = run_mc_mld(eng, NFAPowerModel, ipopt_solver)
    ...
    @testset "3-bus balanced switch open mld lpubfdiag" begin
        result = run_mc_mld(eng, LPUBFDiagPowerModel, ipopt_solver)
