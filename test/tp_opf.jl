...
@testset "test multi-phase parser" begin
    ...
    @testset "5-bus independent meshed different off-diagonal ac case" begin
        mp_data = ThreePhasePowerModels.parse_file("../test/data/matlab/case5_i_m_c.m")
        result = run_tp_opf(mp_data, PMs.ACPPowerModel, ipopt_solver)
    ...
    @testset "5-bus independent meshed different off-diagonal soc case" begin
        mp_data = ThreePhasePowerModels.parse_file("../test/data/matlab/case5_i_m_c.m")
        result = run_tp_opf(mp_data, PMs.SOCWRPowerModel, ipopt_solver)

        @test result["status"] == :LocalOptimal
        @test isapprox(result["objective"], 45567.2067; atol = 1e-1)

        @test isapprox(result["solution"]["gen"]["1"]["qg"][1],  0.064287; atol = 1e-3)
        @test isapprox(result["solution"]["gen"]["1"]["qg"][2],  0.070123; atol = 1e-3)
        @test isapprox(result["solution"]["gen"]["1"]["qg"][3],  0.064287; atol = 1e-3)
