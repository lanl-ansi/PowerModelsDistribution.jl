...
@testset "matrix shunts ACP/ACR/IVR" begin
    data = transform_data_model(ravens_case2_mxshunt_2)
    
    println("<debug> "*string(data["shunt"]))
    shunt = data["shunt"]["1"]

    @test(isa(shunt["gs"], Matrix))
    @test(isa(shunt["bs"], Matrix))

    @test_broken(all([shunt["bs"][c,d]!=0 for c in 1:3, d in 1:3 if c!=d]))
    ...
    @test_broken(!isapprox(result_acp["solution"]["bus"]["2"]["vm"], result_acp_diag["solution"]["bus"]["2"]["vm"]))
