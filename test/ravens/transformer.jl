...
@testset "transformers" begin
    @testset "test transformer acp pf" begin
        @testset "2w transformer acp pf yy" begin
            ...
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.87451, 0.8613, 0.85348], Inf) <= 1.5E-1
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([-0.1, -120.4, 119.8]), Inf) <= 0.5
        end
        ...
        @testset "2w transformer acp pf dy_lead" begin
            ...
            result = solve_mc_pf(math, ACPUPowerModel, ipopt_solver; solution_processors=[sol_data_model!], make_si=false)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.87391, 0.86055, 0.85486], Inf) <= 1.5E-1
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([29.8, -90.4, 149.8]), Inf) <= 1.25
        end
        ...
        @testset "2w transformer acp pf dy_lag" begin
            ...
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.92092, 0.91012, 0.90059], Inf) <= 1.5E-1
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([-30.0, -150.4, 89.8]), Inf) <= 0.5
        end
    end
    ...
    @testset "test transformer ivr pf" begin
        ...
        @testset "2w transformer ivr pf dy_lag" begin
            ...
            x = wrap_to_range.(result["solution"]["bus"]["3"]["va"])
            @test norm(x - deg2rad.([-30.0, -150.4, 89.8]), Inf) <= 0.5
        end
    end
    ...
    @testset "three winding transformer pf" begin
        @testset "3w transformer ac pf dyy - all non-zero"  begin
            ...
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.9318, 0.88828, 0.88581], Inf) <= 1.5E-1 #evaluated 0.15 = 0.226
            @test_broken norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([30.1, -90.7, 151.2]), Inf) <= 0.25
        end
        ...
        @testset "3w transformer ac pf dyy - some non-zero" begin
            ...
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.93876, 0.90227, 0.90454], Inf) <= 1.5E-1 #0.91 == 0.15
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([31.6, -88.8, 153.3]), Inf) <= 0.5 #5.78 == 0.5
        end
        ...
        @testset "3w transformer ac pf dyy - all zero" begin
            ...
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.97047, 0.93949, 0.946], Inf) <= 1.5E-1 #771==0.15
            @test_broken norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([30.6, -90.0, 151.9]), Inf) <= 0.5
        end
        ...
        @testset "3w transformer ac pf dyy - %loadloss=0" begin
            ...
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.969531, 0.938369, 0.944748], Inf) <= 1.5E-1 #2.5 <= 0.15
            @test_broken norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([30.7, -90.0, 152.0]), Inf) <= 0.5 #5.75 <= 0.15
        end

        @testset "3w transformer ac pf center-tap" begin
            #something here crashes. PhaseCode.s1N not found. pretty sure owen changed something about the phase code names?
            math = transform_data_model(RLF_trans_3w_center_tap) # my version with fixed resistance
            result_math = solve_mc_pf(math, ACPUPowerModel, ipopt_solver; make_si=false)
            result = transform_solution(result_math; make_si = false)
    ...
    @testset "oltc tests" begin
        @testset "2w transformer acp opf_oltc yy" begin
            ...
            @test_broken norm(tap(1,pm)-[0.95, 0.95, 0.95], Inf) <= 1E-4 #0.499 <= 0.0001
            @test_broken norm(tap(2,pm)-[1.05, 1.05, 1.05], Inf) <= 1E-4 #0.499 <= 0.0001


            @test_broken result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED

            # then check whether voltage is what OpenDSS expects for those values
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[1.0352, 1.022, 1.0142], Inf) <= 1E-4 #0.1396 <= 0.0001
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([-0.1, -120.4, 119.8]), Inf) <= 0.5
        end
    end
    ...
    @testset "linearized transformers" begin
        ...
        @testset "3w_dyy_1" begin
            ...
            result = solve_mc_opf(math, LPUBFDiagPowerModel, ipopt_solver)
            @test_broken result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED
            @test_broken norm(result["solution"]["bus"]["3"]["w"]-[0.86095, 0.81344, 0.80480], Inf) <= 1E-4 #0.635 <= 0.0001
        end

        @testset "3w_dyy_2" begin
            math = transform_data_model(ravens_ut_trans_3w_dyy_2)
            result = solve_mc_opf(math, LPUBFDiagPowerModel, ipopt_solver)
            @test_broken result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED #false
            @test_broken norm(result["solution"]["bus"]["3"]["w"]-[0.87086, 0.83270, 0.83208], Inf) <= 1E-4 #0.6 <= 0.0001
        end
        ...
        @testset "2w_dy_lead_small_series_impedance" begin
            ...
            @test_broken result1["termination_status"] == LOCALLY_SOLVED
            @test_broken result2["termination_status"] == LOCALLY_SOLVED
    ...
    @testset "voltage regulator control" begin
        @testset "regcontrol_acp" begin
            ...
            @test_broken all(isapprox.(result["solution"]["bus"]["12"]["vm"] ./ vbase, [0.0165, 0.01582, 0.0158]; atol=2e-2))
            @test all(isapprox.(result["solution"]["bus"]["12"]["va"], deg2rad.([29.99, -90.00, 149.99]); atol=3e-1))

            @test_broken all(isapprox.(result["solution"]["transformer"]["3"]["pf"], [1.22, 1.08, 1.10]; atol=3e-2))
            @test_broken all(isapprox.(result["solution"]["transformer"]["3"]["qf"], [1.22, 1.08, 1.10]; atol=3e-2))
            @test_broken all(isapprox.(result["solution"]["transformer"]["3"]["pt"], [1.22, 1.08, 1.10]; atol=3e-2))
            @test_broken all(isapprox.(result["solution"]["transformer"]["3"]["qt"], [1.22, 1.08, 1.10]; atol=3e-2))
        end
        ...
        @testset "regcontrol_acr" begin
            ...
            @test_broken all(isapprox.(result["solution"]["bus"]["12"]["vm"] ./ vbase, [0.0165, 0.0158, 0.0158]; atol=2e-2))
        ...
        @testset "regcontrol_lpubfdiag" begin
            ...
            @test_broken all(isapprox.(result["solution"]["bus"]["12"]["vm"] ./ vbase, [0.016, 0.016, 0.016]; atol=3e-2))
        ...
        @testset "regcontrol_fbs" begin
            ...
            @test_broken all(isapprox.(result["solution"]["bus"]["12"]["vm"] ./ vbase, [0.019, 0.016, 0.016]; atol=3e-2))
        ...
        @testset "regcontrol_fotp" begin
            ...
            @test_broken all(isapprox.(result["solution"]["bus"]["12"]["vm"] ./ vbase, [0.046, 0.038, 0.039]; atol=3e-2))
        ...
        # @testset "3w transformer acp opf center-tap" begin #throws phase code key not found again
        ...
        # @testset "3w transformer acr opf center-tap" begin #throws phase code key not found again
        #     math = transform_data_model(ravens_trans_3w_center_tap)
        ...
        # @testset "3w transformer ivr opf center-tap" begin #throws phase code key not found again
        #     math = transform_data_model(ravens_trans_3w_center_tap)
    ...
    #     @testset "3w transformer fotp opf center-tap" begin #throws phase code key not found again
    #         math = transform_data_model(ravens_trans_3w_center_tap)
    ...
    #     @testset "3w transformer fotr opf center-tap" begin  #throws phase code key not found again
    #         math = transform_data_model(ravens_trans_3w_center_tap)
    ...
    #     @testset "3w transformer fbs center-tap" begin #throws phase code key not found again
    #         math = transform_data_model(ravens_trans_3w_center_tap)
    ...
    #     @testset "3w transformer lpubfdiag opf center-tap" begin #throws phase code key not found again
    #         math = transform_data_model(ravens_trans_3w_center_tap)
    #         apply_voltage_bounds_math!(math; vm_lb=0.95, vm_ub=1.05)
    ...
    @testset "transformer SOC relaxations" begin
        @testset "2w_yy" begin
            ...
            @test_broken norm(result["solution"]["bus"]["3"]["vm"]-[0.82944, 0.86067, 0.72315], Inf) <= 2E-1
        end
        ...
        # @testset "3w_center_tap" begin #phase code key not found
        #     math = transform_data_model(ravens_trans_3w_center_tap)
        #     apply_voltage_bounds_math!(math; vm_lb=0.95, vm_ub=1.05)
        #     result = solve_mc_opf(math, SOCConicUBFPowerModel, scs_solver; solution_processors=[sol_data_model!], make_si=false)
...
# #=         @testset "trans_3w_center_tap" begin #phase code key
#             math = transform_data_model(ravens_trans_3w_center_tap)
#             data = deepcopy(math)
...
#         end
#  =#
#         @testset "dist_transformer" begin #phase code key
...
#             @test norm(result["solution"]["bus"]["4"]["vm"]-[0.9990740842103211], Inf) <= 1.5E-1
#             @test norm(result["solution"]["bus"]["4"]["va"]-deg2rad.([-0.39064739635881085]), Inf) <= 0.5
#             @test norm(result["solution"]["bus"]["4_l"]["vm"]-[0.9990723339621554], Inf) <= 1.5E-1
#             @test norm(result["solution"]["bus"]["4_l"]["va"]-deg2rad.([-0.3907533731198626]), Inf) <= 0.5
#         end
#     end
     end
end
