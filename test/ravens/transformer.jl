...
@testset "transformers" begin
    ...
    @testset "three winding transformer pf" begin
        @testset "3w transformer ac pf dyy - all non-zero"  begin
            ...
            @test norm(result["solution"]["bus"]["3"]["vm"]-[0.9318, 0.88828, 0.88581], Inf) <= 1.5E-1 #evaluated 0.15 = 0.226
        ...
        @testset "3w transformer ac pf dyy - some non-zero" begin
            ...
            @test norm(result["solution"]["bus"]["3"]["vm"]-[0.93876, 0.90227, 0.90454], Inf) <= 1.5E-1 #0.91 == 0.15
        ...
        @testset "3w transformer ac pf dyy - all zero" begin
            ...
            @test norm(result["solution"]["bus"]["3"]["vm"]-[0.97047, 0.93949, 0.946], Inf) <= 1.5E-1 #771==0.15
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([30.6, -90.0, 151.9]), Inf) <= 0.5
        end
        ...
        @testset "3w transformer ac pf dyy - %loadloss=0" begin
            ...
            @test norm(result["solution"]["bus"]["3"]["vm"]-[0.969531, 0.938369, 0.944748], Inf) <= 1.5E-1 #2.5 <= 0.15
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([30.7, -90.0, 152.0]), Inf) <= 0.5 #5.75 <= 0.15
        end
    ...
    @testset "oltc tests" begin
        @testset "2w transformer acp opf_oltc yy" begin
            ...
            @test norm(tap(1,pm)-[0.95, 0.95, 0.95], Inf) <= 1E-4 #0.499 <= 0.0001
            @test norm(tap(2,pm)-[1.05, 1.05, 1.05], Inf) <= 1E-4 #0.499 <= 0.0001


            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED

            # then check whether voltage is what OpenDSS expects for those values
            @test norm(result["solution"]["bus"]["3"]["vm"]-[1.0352, 1.022, 1.0142], Inf) <= 1E-4 #0.1396 <= 0.0001
            @test norm(wrap_to_range.(result["solution"]["bus"]["3"]["va"])-deg2rad.([-0.1, -120.4, 119.8]), Inf) <= 0.5
        end
    end
    ...
    @testset "linearized transformers" begin
        ...
        @testset "3w_dyy_1" begin
            ...
            result = solve_mc_opf(math, LPUBFDiagPowerModel, ipopt_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED
            @test norm(result["solution"]["bus"]["3"]["w"]-[0.86095, 0.81344, 0.80480], Inf) <= 1E-4 #0.635 <= 0.0001
        end

        @testset "3w_dyy_2" begin
            math = transform_data_model(ravens_ut_trans_3w_dyy_2)
            result = solve_mc_opf(math, LPUBFDiagPowerModel, ipopt_solver)
            @test result["termination_status"] == LOCALLY_SOLVED || result["termination_status"] == ALMOST_LOCALLY_SOLVED #false
            @test norm(result["solution"]["bus"]["3"]["w"]-[0.87086, 0.83270, 0.83208], Inf) <= 1E-4 #0.6 <= 0.0001
        end
