...
@testset "transformers" begin
    @testset "test transformer acp pf" begin
        @testset "2w transformer acp pf yy" begin
            ...
            @test norm(result["bus"]["3"]["vm"]-[2.26754, 2.268515, 2.263806], Inf) <= 1.5E-2
            @test norm(result["bus"]["3"]["va"]-[-43.774, -163.85, 76.2628], Inf) <= 0.1
        end
        ...
        @testset "2w transformer acp pf dy_lead" begin
            ...
            result = solve_mc_pf(math, ACPUPowerModel, ipopt_solver; solution_processors=[sol_data_model!], make_si=false)
            result = transform_solution(result["solution"], math, make_si=true)
            @test norm(result["bus"]["3"]["vm"]-[2.26754, 2.268515, 2.263806], Inf) <= 1.5E-5
            @test norm(result["bus"]["3"]["va"]-[-73.7194, 166.1574, 46.1977], Inf) <= 0.1
        end
    ...
    @testset "test transformer ivr pf" begin
        @testset "2w transformer ivr pf yy" begin
            ...
            result = transform_solution(result["solution"], math, make_si=true)
            @test norm(result["bus"]["3"]["vm"]-[5.1058, 5.5512, 5.98247], Inf) <= 1.5E-2
            @test norm(result["bus"]["3"]["va"]-[-108.8580, -145.64931, 117.8380], Inf) <= 0.1
        end
        ...
        @testset "2w transformer ivr pf dy_lead" begin
            ...
            result = solve_mc_pf(math, IVRUPowerModel, ipopt_solver; solution_processors=[sol_data_model!], make_si=false)
            result = transform_solution(result["solution"], math, make_si=true)
            @test norm(result["bus"]["3"]["vm"]-[5.123, 5.5482, 5.962], Inf) <= 1.5E-2
            @test norm(result["bus"]["3"]["va"]-[-57.975, 147.7116, 46.999], Inf) <= 0.1
        end
        ...
        @testset "2w transformer ivr pf dy_lag" begin
            ...
            result = transform_solution(result["solution"], math, make_si=true)
            @test norm(result["bus"]["3"]["vm"]-[5.123, 5.5482, 5.962], Inf) <= 1.5E-2
            @test norm(result["bus"]["3"]["va"]-[-57.9757, 147.7116, 46.9990], Inf) <= 0.1
        end
    end
    ...
    @testset "test transformer acr pf" begin
        @testset "2w transformer acr pf yy" begin
            ...
            result = transform_solution(result["solution"], math, make_si=true)
            @test norm(result["bus"]["3"]["vm"]-[2.26754, 2.268515, 2.263806], Inf) <= 1.5E-2
            @test norm(result["bus"]["3"]["va"]-[-43.7745, -163.8520, 76.2629], Inf) <= 0.1
        end
    ...
    # @testset "three winding transformer pf" begin
        ...
        #     result = solve_mc_pf(math, ACPUPowerModel, ipopt_solver; make_si=false)
        #     @test norm(result["bus"]["3"]["vm"]-[0.9318, 0.88828, 0.88581], Inf) <= 1.5E-5
        #     @test norm(result["bus"]["3"]["va"]-[30.1, -90.7, 151.2], Inf) <= 0.1
        # end
        ...
        #     @test norm(result["bus"]["3"]["vm"]-[0.93876, 0.90227, 0.90454], Inf) <= 1.5E-5
        #     @test norm(result["bus"]["3"]["va"]-[31.6, -88.8, 153.3], Inf) <= 0.1
        # end

        # @testset "3w transformer ac pf dyy - all zero" begin
        #     math = transform_data_model(ravens_ut_trans_3w_dyy_3)
        #     result = solve_mc_pf(math, ACPUPowerModel, ipopt_solver; make_si=false)
        #     @test norm(result["bus"]["3"]["vm"]-[0.97047, 0.93949, 0.946], Inf) <= 1.5E-5
        #     @test norm(result["bus"]["3"]["va"]-[30.6, -90.0, 151.9], Inf) <= 0.1
        # end

        # @testset "3w transformer ac pf dyy - %loadloss=0" begin
        #     math = transform_data_model(ravens_ut_trans_3w_dyy_3_loadloss)
        #     result = solve_mc_pf(math, ACPUPowerModel, ipopt_solver; make_si=false)
        #     @test norm(result["bus"]["3"]["vm"]-[0.969531, 0.938369, 0.944748], Inf) <= 1.5E-5
        #     @test norm(result["bus"]["3"]["va"]-[30.7, -90.0, 152.0], Inf) <= 0.1
        # end
        ...
        #     @test all(isapprox.(sum(result["load"]["l3"]["pd"])*sbase, 20.0; atol=1E-5))
        #     @test all(isapprox.(sum(result["generator"]["g1"]["pg_bus"])*sbase, 7.0; atol=9E-4))
        #     @test all(isapprox.(sum(result["solar"]["pv1"]["pg_bus"])*sbase, 3.0; atol=9E-4))
        # end
    # end
    ...
    @testset "oltc tests" begin
        @testset "2w transformer acp opf_oltc yy" begin
            ...
            @test norm(solution["bus"]["3"]["vm"]-[0.8,0.8,0.8], Inf) <= 1E-4
            @test norm(solution["bus"]["3"]["va"]-[-9.16, -129.24, 110.8], Inf) <= 0.1
        end
    end
    ...
    @testset "linearized transformers" begin
        @testset "2w_dy_lead" begin
            ...
            result = transform_solution(result["solution"], math, make_si=true)
            @test_skip norm(result["bus"]["3"]["w"]-[0.76674, 0.74840, 0.73846], Inf) <= 1E-4
        end

        # @testset "3w_dyy_1" begin
        #     math = transform_data_model(ravens_ut_trans_3w_dyy_1)
        #     result = solve_mc_opf(math, LPUBFDiagPowerModel, ipopt_solver)
        #     @test norm(result["bus"]["3"]["w"]-[0.86095, 0.81344, 0.80480], Inf) <= 1E-4
        # end

        # @testset "3w_dyy_2" begin
        #     math = transform_data_model(ravens_ut_trans_3w_dyy_2)
        #     result = solve_mc_opf(math, LPUBFDiagPowerModel, ipopt_solver)
        #     @test norm(result["bus"]["3"]["w"]-[0.87086, 0.83270, 0.83208], Inf) <= 1E-4
        # end
        ...
        @testset "2w_dy_lead_small_series_impedance" begin
            ...
            result1 = transform_solution(result1["solution"], math1, make_si=true)
            result2 = transform_solution(result2["solution"], math2, make_si=true)

            @test norm(result1["bus"]["1"]["vm"]-result2["bus"]["1"]["vm"], Inf) <= 1.2E-3

            @test norm(result1["line"]["line1"]["pf"]-result2["line"]["line1"]["pf"], Inf) <= 1E-1
        end
    end
    ...
    @testset "voltage regulator control" begin
        @testset "regcontrol_acp" begin
            ...
            result = transform_solution(result["solution"], math, make_si=false)

            @test isapprox(sum(result["voltage_source"]["source"]["pg"]), 0; atol=1)
            @test isapprox(sum(result["voltage_source"]["source"]["qg"]), 0; atol=1)

            sourcebus_id = math["bus_lookup"]["sourcebus"]
            vbase = math["bus"][string(sourcebus_id)]["vbase"]

            @test all(isapprox.(result["bus"]["rg60"]["vm"] ./ vbase, [0.0165, 0.01582, 0.0158]; atol=2e-2))
            @test all(isapprox.(result["bus"]["rg60"]["va"], [29.99, -90.00, 149.99]; atol=3e-1))

            @test all(isapprox.(result["transformer"]["reg1"]["tap"][2], [1.10,1.05,1.05]; atol=2e-2))
        end
        ...
        @testset "regcontrol_acr" begin
            ...
            @test all(isapprox.(result["bus"]["rg60"]["vm"] ./ vbase, [0.0165, 0.0158, 0.0158]; atol=2e-2))
            @test all(isapprox.(result["bus"]["rg60"]["va"], [30.00, -90.00, 150.00]; atol=3e-1))

            @test all(isapprox.(result["transformer"]["reg1"]["tap"][2], [1.100, 1.050, 1.050]; atol=2e-2))
        end
        ...
        @testset "regcontrol_lpubfdiag" begin
            ...
            @test isapprox(sum(result["voltage_source"]["source"]["pg"]), 10; atol=10)
            @test isapprox(sum(result["voltage_source"]["source"]["qg"]), -5; atol=10)

            sourcebus_id = math["bus_lookup"]["sourcebus"]
            vbase = math["bus"][string(sourcebus_id)]["vbase"]

            @test all(isapprox.(result["bus"]["rg60"]["vm"] ./ vbase, [0.016, 0.016, 0.016]; atol=3e-2))

            @test all(isapprox.(result["transformer"]["reg1"]["tap"][2], [1.10, 0.96, 0.96]; atol=3e-2))
        end
        ...
        @testset "regcontrol_fbs" begin
            ...
            @test all(isapprox.(result["bus"]["rg60"]["vm"] ./ vbase, [0.019, 0.016, 0.016]; atol=3e-2))
            @test all(isapprox.(result["bus"]["rg60"]["va"], [30.00, -94.04, 149.99]; atol=5e-1))

            @test all(isapprox.(result["transformer"]["reg1"]["tap"][2], [1.22, 1.08, 1.10]; atol=3e-2))
        end
        ...
        @testset "regcontrol_fotp" begin
            ...
            @test all(isapprox.(result["bus"]["rg60"]["vm"] ./ vbase, [0.046, 0.038, 0.039]; atol=3e-2))
            @test all(isapprox.(result["bus"]["rg60"]["va"], [30.00, -90.05, 149.93]; atol=5e-1))

            @test all(isapprox.(result["transformer"]["reg1"]["tap"][2], [1.10, 1.06, 1.06]; atol=3e-1))
        end
        ...
        @testset "regcontrol_fotr" begin
            ...
            @test all(isapprox.(result["bus"]["rg60"]["vm"] ./ vbase, [0.046, 0.038, 0.039]; atol=3e-2))
            @test all(isapprox.(result["bus"]["rg60"]["va"], [30.00, -94.37, 148.76]; atol=5e-1))
        ...
        #     @test all(isapprox.(result["load"]["l3"]["pd"]*sbase, [10.0, 10.0]; atol=1E-5))
        #     @test all(isapprox.(result["generator"]["g1"]["pg_bus"]*sbase, [0.0, 0.0]; atol=9E-4))
        #     @test all(isapprox.(result["solar"]["pv1"]["pg_bus"]*sbase, [0.0, 0.0]; atol=9E-4))
        #     @test all(isapprox.(result["bus"]["tn_1"]["vm"], [1.045, 1.05]; atol=1E-3))
        #     @test all(isapprox.(result["bus"]["tm_2"]["va"], [-120.1, 59.9]; atol=0.1))
        #     @test all(isapprox.(result["bus"]["tn_6"]["va"], [119.9, -60.1]; atol=0.1))
        # end
    ...
    #         @test all(isapprox.(result["generator"]["g1"]["pg_bus"]*sbase, [0.0, 0.0]; atol=9E-4))
    #         @test all(isapprox.(result["solar"]["pv1"]["pg_bus"]*sbase, [0.0, 0.0]; atol=9E-4))
    #         @test all(isapprox.(result["bus"]["tn_1"]["vm"], [1.045, 1.05]; atol=5E-3))
    #     end
    end
    ...
    @testset "transformer SOC relaxations" begin
        @testset "2w_yy" begin
            ...
            result = transform_solution(result["solution"], math, make_si=false)
            @test norm(result["bus"]["3"]["vm"]-[0.8, 0.8, 0.8], Inf) <= 2E-2
        end
        ...
        @testset "2w_dy_lead" begin
            ...
            result = transform_solution(result["solution"], math, make_si=true)
            @test_skip norm(result["bus"]["3"]["vm"]-[0.87391, 0.86054, 0.85485], Inf) <= 4.2E-2
        end
        ...
        @testset "3w_dyy_1" begin
            ...
            result = solve_mc_opf(math, SOCConicUBFPowerModel, scs_solver; solution_processors=[sol_data_model!], make_si=false)
            result = transform_solution(result["solution"], math, make_si=true)
            @test_skip norm(result["bus"]["3"]["vm"]-[0.93180, 0.88827, 0.88581], Inf) <= 1E-1
        end
        ...
        #     @test all(isapprox.(result["load"]["l2a"]["pd"]*sbase, 12.0; atol=1E-5))
        #     @test all(isapprox.(result["load"]["l3"]["pd"]*sbase, [10.0, 10.0]; atol=1E-5))
        #     @test all(isapprox.(result["bus"]["tn_1"]["vm"], [1.045, 1.05]; atol=5E-3))
        # end
    end
    ...
    #         @test norm(result["bus"]["4"]["vm"]-[0.9990740842103211], Inf) <= 1.5E-5
    #         @test norm(result["bus"]["4"]["va"]-[-0.39064739635881085], Inf) <= 0.1
    #         @test norm(result["bus"]["4_l"]["vm"]-[0.9990723339621554], Inf) <= 1.5E-5
    #         @test norm(result["bus"]["4_l"]["va"]-[-0.3907533731198626], Inf) <= 0.1
    #     end
    # end
end
