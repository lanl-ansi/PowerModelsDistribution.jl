...
@testset "test mld" begin
    ...
    @testset "transformer lpubfdiag mld" begin
        ...
        @test_broken result["termination_status"] == NORM_LIMIT
        #ACTUAL: @test result["termination_status"] == OTHER_ERROR

        @test_broken isapprox(result["objective"], 36; atol=1) #evaluates 0.43==36
        #ACTUAL: @test isapprox(result["objective"], 46; atol=1)
        @test_broken isapprox(result["solution"]["load"]["1"]["status"], .75; atol=1e-3) #0.75 == 1
