...
@testset "matrix shunts ACP/ACR/IVR" begin
    data = PMD.parse_file("data/opendss/case_mxshunt_2.dss"; data_model="mathematical")
    shunt = data["shunt"]["1"]
    ...
    sol_acp_diag = run_mc_pf(data_diag_shunt, PMs.ACPPowerModel, ipopt_solver)
    sol_acp = run_mc_pf(data, PMs.ACPPowerModel, ipopt_solver)
    sol_acr = run_mc_pf(data, PMs.ACRPowerModel, ipopt_solver)
    sol_iv = run_mc_pf_iv(data, PMs.IVRPowerModel, ipopt_solver)
